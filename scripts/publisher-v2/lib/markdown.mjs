...
export function replaceInputs(body, fields) {
  return body.replace(
    /`?INPUT\[(.+?):([A-Za-z0-9_.-]+)\]`?/g,
    (_match, _control, fieldName) => {
      const key = String(fieldName).trim();
      const value = fields[key];
...
export function removeCallouts(body, calloutNames) {
  ...
  for (
    ...
    if (
      match &&
      names.includes(match[1].toLowerCase())
    ) {
      index += 1;
